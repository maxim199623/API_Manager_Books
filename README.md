...
- TLS-сертификат и ключ для production-запуска
...
Подробная инструкция по dev-запуску, локальным переменным окружения, self-signed TLS-сертификатам и проверкам находится в [docs/development.md](docs/development.md).

В dev-режиме при запуске на `127.0.0.1` или `localhost` приложение может создать локальный self-signed сертификат автоматически. Клиенту для доверия нужен `cert.pem`; `key.pem` является приватным ключом сервера и клиенту не передается.
